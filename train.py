...
def train(stage, steps):
    ...
    if ddp:
        ...
        )  # rank on this node, 0 to num_gpu_per_node-1
    ...
    model.convnet.load_state_dict(
        state_dict_convnet
    )  # load_state_dict matches key and tensor shape
    ...
    model = torch.compile(
        model
    )  # compiler for Neural networks, compile the model to make it faster
    ...
    # This is important when training with mix precision,
    ...
    for step in range(max_steps):
        ...
        )  # (B, 3, 224, 224), (Num of shots in this point,)
        ...
        if master_process:
            print(
                f"step {step:5d} | loss: {loss_accum.item():.6f} | lr {lr:.4e} | norm: {norm:.4f} | dt: {dt*1000:.2f}ms | examples: {examples_processed:5d} | examples/sec: {examples_per_sec:.2f}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "stage", help="which training stage to run, stage0 or stage1 or stage2"
    )
    parser.add_argument(
        "--steps", type=int, default=10, help="number of iteration steps to run"
    )
