...
class DataLoader:
    def __init__(
        self, process_rank=0, num_processes=1, mode="train", infer_folder=None
    ):
        ...
        self.not_empty = (
            True  # this indicates whether there are still pts in the loader during eval
        )
        ...
        self.files = sorted(
            [f for f in os.listdir(self.imgfolder) if f.endswith(".jpg")]
        )
        print(f"{len(self.files)} images from {self.imgfolder}")

        if self.mode == "train":
            self.frame_target = np.loadtxt(
                "Dataset/TrainingStage1_label.csv", delimiter=",", dtype=str
            )
        elif self.mode == "val":
            self.frame_target = np.loadtxt(
                "Dataset/ValStage1_label.csv", delimiter=",", dtype=str
            )
        elif self.mode == "test":
            self.frame_target = np.loadtxt(
                "Dataset/TestStage1_label.csv", delimiter=",", dtype=str
            )
        else:  # inference
            ...
            img_process = transforms.Compose(
                [
                    transforms.RandomResizedCrop(
                        224, scale=[0.85, 1], ratio=[0.75, 1.25]
                    ),
...
class DataLoaderStage1(DataLoader):
    ...
    def next_batch(self):
        ...
        ]  # self.files is list not numpy array
...
class DataLoaderStage2(DataLoader):
    def __init__(
        ...
        if self.mode == "train":
            target = np.loadtxt(
                "Dataset/TrainingStage2_label.csv", delimiter=",", dtype=str
            )
        elif self.mode == "val":
            target = np.loadtxt("Dataset/ValStage2_label.csv", delimiter=",", dtype=str)
        elif self.mode == "test":
            target = np.loadtxt(
                "Dataset/TestStage2_label.csv", delimiter=",", dtype=str
            )
        else:  # infer
            target = None
        ...
        for i in range(2, 6):
            frame_label[frame_label == i] = (
                1  # we'll set anything besides during_pt as out_of_pt
            )
        ...
        for a, b in zip(
            in_point_out_point_indices[:-1:2], in_point_out_point_indices[1::2]
        ):
            self.indices.append(
                list(range(a.item() - 4, b.item() + 4))
            )  # we add a bit more buffer, 4 frames

        # TODO: you might want to add a preprocessing step to check for this condition
        assert len(self.indices) == len(
            self.target
        ), "number of points must match number of points in target"
    ...
    def next_batch(self):
        ...
        if self.target is not None:  # not during inference
            ...
            point_token = torch.tensor(
                [POINTSEPARATOR], dtype=torch.long, device=y.device
            )
            y = torch.cat(
                (point_token, y, point_token), dim=0
            )  # (Num of shots in this point+2,)
